...
namespace Ied
{
    public partial class PtovForm : Form
    {
        public PtovForm()
        {
            InitializeComponent();
        ...
        private void PtovForm_Load(object sender, EventArgs e)
        {
        ...
        private void Initialize()
        {
            for (int i = 0; i < 3; i++)
            {
                cbLevels[i].Checked = conf59.level[i].enabled;
                tbVoltage[i].Text = ((double)conf59.level[i].pickup).ToString("0.00");
                tbTimeDelay[i].Text = ((double)conf59.level[i].timeDelay).ToString("0.00");
            }

            foreach (var cb in cbLevels)
                levelChange(cb, EventArgs.Empty);

            if (conf59.timeType == TimeType.cycle)
        ...
        private void levelChange(object sender, EventArgs e)
        {
            ...
            conf59.level[idx].enabled = cbLevels[idx].Checked;
        ...
        private void RbVolts_CheckedChanged(object sender, EventArgs e)
        {
            if (RbVolts.Checked)
            {
                conf59.voltageType = VoltageType.Volts;
                for (int i = 0; i < 3; i++)
                {
                    lbVoltage[i].Text = "[V]";
                    tbVoltage[i].Text = ((double)conf59.level[i].pickup).ToString("0.00");
            ...
            {
                conf59.voltageType = VoltageType.Pu;
                for (int i = 0; i < 3; i++)
                {
                    lbVoltage[i].Text = "[Pu]";
                    tbVoltage[i].Text = ((double)conf59.level[i].pickup / (double)main.general.NominalVoltage).ToString("0.00");
                }
            }
        ...
        private void RbSeconds_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                conf59.timeType = TimeType.seconds;
                for (int i = 0; i < 3; i++)
                {
                    lbTimeDelay[i].Text = "[s]";
                    tbTimeDelay[i].Text = ((double)conf59.level[i].timeDelay).ToString("0.0000");
                }
            }
            else
            {
                conf59.timeType = TimeType.cycle;
                for (int i = 0; i < 3; i++)
                {
                    lbTimeDelay[i].Text = "[cy]";
                    tbTimeDelay[i].Text = ((double)conf59.level[i].timeDelay * (double)main.general.frequency).ToString("0.00");
                }
            }
        ...
        private void TbPickupValidated(object sender, EventArgs e)
        {
            ...
            conf59.level[idx].pickup = local.Text;
            if (conf59.voltageType == VoltageType.Pu)
            {
                conf59.level[idx].pickup = (double)conf59.level[idx].pickup * (double)main.general.NominalVoltage;
                local.Text = ((double)conf59.level[idx].pickup / (double)main.general.NominalVoltage).ToString("0.00");
            }
            else
                local.Text = ((double)conf59.level[idx].pickup).ToString("0.00");
            updateGraph();
            local.Validated += TbPickupValidated;
        }
        ...
        private void TbTripValidated(object sender, EventArgs e)
        {
            ...
            conf59.level[idx].timeDelay = local.Text;
            if (conf59.timeType == TimeType.cycle)
            {
                conf59.level[idx].timeDelay = (double)conf59.level[idx].timeDelay / (double)main.general.frequency;
                local.Text = ((double)conf59.level[idx].timeDelay * (double)main.general.frequency).ToString("0.00");
            }
            else local.Text = ((double)conf59.level[idx].timeDelay).ToString("0.0000");
