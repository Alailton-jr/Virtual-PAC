#ifndef SEND_H  // If not defined
#define SEND_H  // Define it
...
int32_t createSocket(struct eth_t *eth, char* ifName) {
    ...
    if (ioctl(eth->socket, SIOCGIFINDEX, &if_idx) < 0) {
        perror("ioctl");
        close(eth->socket);
        return 1;
    }
    eth->if_index = if_idx.ifr_ifindex;
    ...
    // bind(eth->socket, (struct sockaddr *)&eth->bind_addr, sizeof(eth->bind_addr));
