...
static PyObject* pyTransientReplay(PyObject* self, PyObject* args){
    ...
    if (shm_data.ptr != NULL){
        transientData_t *data = (transientData_t*)shm_data.ptr;
...
static PyObject* pySequenceReplay(PyObject* self, PyObject* args){
    PyObject *py_arr = NULL; //numpy array
    PyObject *py_seqNum, *py_smpCountPos, *py_asduLength, *py_allDataPos, *py_interGap, *py_smpPerSeq, *py_smpRate, *py_freq, *py_n_channels, *py_nasdu; // uint16_t
    PyObject *py_structShmName, *py_arrShmName, *py_frameShmName, *py_frame; // uint8_t[]

    // Parse the arguments
    if(!PyArg_ParseTuple(args, "OOOOOOOOOOOOOOO", &py_arr, &py_seqNum, &py_smpCountPos, &py_asduLength, &py_allDataPos, &py_interGap, &py_smpPerSeq, &py_smpRate, &py_freq, &py_n_channels, &py_nasdu, &py_structShmName, &py_arrShmName, &py_frameShmName, &py_frame)){
        return NULL;
    }
    ...
    shm_setup_s shm_data = openSharedMemory(structShmName, sizeof(transientData_t));
    if (shm_data.ptr != NULL){
        sequenceData_t *data = (sequenceData_t*)shm_data.ptr;
        shm_setup_s shm_arr = openSharedMemory((char*)data->arrShmName, 0);
        if (shm_arr.ptr != NULL){
            deleteSharedMemory(&shm_arr);
        }
    ...
    data->n_channels = (uint16_t)PyLong_AsLong(py_n_channels);
    data->n_asdu = (uint16_t)PyLong_AsLong(py_nasdu);
    data->seqNum = (uint16_t)PyLong_AsLong(py_seqNum);
    data->smpRate = (uint16_t)PyLong_AsLong(py_smpRate);
    data->freq = (uint16_t)PyLong_AsLong(py_freq);

    for(int i=0; i<data->seqNum; i++){
        data->smpPerSeq[i] = (uint64_t)PyLong_AsLong(PyList_GetItem(py_smpPerSeq, i));
    }
