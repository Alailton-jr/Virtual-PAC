...
int main(int argc, char *argv[])
{
    ...
    if (argc != 5){
        printf("Usage: ./ptov <relay number> <pickup> <td>\n");
        cleanup(1);
    }
    ...
    char tripName[] = "PTOV_TRIP_0";
    char pickupName[] = "PTOV_PICKUP_0";
    snprintf(tripName,sizeof(tripName) ,"PTOV_TRIP_%d", (int) strtod(argv[1], &endptr));
    snprintf(pickupName,sizeof(pickupName) , "PTOV_PICKUP_%d", (int) strtod(argv[1], &endptr));
