...
void *snifferThread(void *threadInfo) {
    ...
    ThreadPool pool;
    thread_pool_init(&pool, snifferThread->nThread, snifferThread->nTask);
    
    // Initialize the frameCaptured
    processThread_t *frameInfo = (processThread_t*)malloc((snifferThread->nTask+2)*sizeof(processThread_t));
    uint8_t **frames = (uint8_t**)malloc((snifferThread->nTask+2)*sizeof(uint8_t*));
    ...
    while(!snifferThread->stop) {
        ...
        if ((rx_bytes > 0) && (rx_bytes < eth.rx_size)) {
            ...
            idxThreads++;
            if (idxThreads == snifferThread->nTask+2) idxThreads = 0;
        }
    }
...
void *timed_snifferThread(void *threadInfo){
    ...
    if (snifferThread->num_sv == 0){ // Search for any SV
        ...
        if (snifferThread->sv == NULL){
            snifferThread->sv = (SampledValue_t*)malloc(snifferThread->max_sv*sizeof(SampledValue_t));
        }
        process_frame = (void (*)(void*))process_frame_any_sv;
        if (snifferThread->save_waveform){
            files = (FILE**)malloc(snifferThread->max_sv*sizeof(FILE*));
        ...
        if (snifferThread->save_waveform){
            files = (FILE**)malloc(snifferThread->num_sv*sizeof(FILE*));
